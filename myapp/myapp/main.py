...
logging.basicConfig(filename='logs/maze.log', level=logging.DEBUG, format='%(asctime)s:%(levelname)s:%(message)s')
...
while not done:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            secondx = -100
            secondy = -100
            logging.info('New maze generated')

        elif SAVE_IM.is_clicked(event):
            rect = M.rect
            sub = wn.subsurface(rect)
            pg.image.save(sub, "C:/Users/HONOR/Documents/myapp/myapp/data/screenshot.jpg")
            logging.info('Screenshot of maze saved')
            ...
            if M.rect.collidepoint(pos):
                if firstx == -100:
                    (firstx, firsty) = pos
                    logging.info('First field of tha path is selected')
                else:
                    (secondx, secondy) = pos
                    logging.info('Second field of tha path is selected')
    ...
    if secondx != -100:
        ...
        logging.info('Path between fields is found')
